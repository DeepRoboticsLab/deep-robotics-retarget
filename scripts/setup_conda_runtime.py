"""Install Bash activation hooks for Conda's C++ runtime on Linux.

Run with the environment's Python, then deactivate and reactivate the environment.
The hooks make native dependencies use Conda's runtime before the system copy.
...
def main():
    ...
    if not conda_prefix or Path(conda_prefix).resolve() != Path(sys.prefix).resolve():
        raise SystemExit("Activate your Conda environment and run this script with its Python.")
    ...
    print("Deactivate and reactivate this environment before running retargeting scripts.")
