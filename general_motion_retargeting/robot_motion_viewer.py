"""MuJoCo-based robot motion viewer.

Provides real-time visualization of retargeted robot motion with optional
human motion overlay, video recording, and camera following.
"""
...
class RobotMotionViewer:
    """Real-time robot motion viewer using MuJoCo passive viewer.

    Supports camera following, human body frame overlay, and offscreen video recording.
    """
    def __init__(self,
                robot_type,
                camera_follow=True,
        ...
        if show_collision_geoms:
            # Move group-3 (collision) geoms to group 1 so they are rendered,
            # and make them semi-transparent for visualization.
            for i in range(self.model.ngeom):
                if self.model.geom_group[i] == 3:
                    self.model.geom_group[i] = 1
                    self.model.geom_rgba[i, 3] = 0.5  # semi-transparent
