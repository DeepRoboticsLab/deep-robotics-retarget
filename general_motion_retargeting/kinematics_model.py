"""Analytical forward kinematics model for robot skeletons.

This module implements a kinematics model parsed from MuJoCo XML files.
It supports forward kinematics computation, DOF-to-quaternion conversion,
and joint limit handling. Used primarily for batch processing of retargeted
motion data to obtain local body positions.
"""
...
class Joint:
    ...
    Supports 0-DoF (fixed), 1-DoF (hinge), and 3-DoF (ball) joints.
    Provides conversion between DOF values and quaternion representations.
    """
...
class KinematicsModel:
    """Analytical kinematics model parsed from MuJoCo XML.
    
    Builds a kinematic tree from the XML file and provides forward kinematics,
    DOF-to-quaternion conversion, and joint limit queries. All computations
    are performed using PyTorch for batch processing support.
    
    Args:
        file_path: Path to the MuJoCo XML file.
